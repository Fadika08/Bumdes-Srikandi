...
// Query untuk mendapatkan daftar UMKM berdasarkan kategori
if ($kategori == 'semua') {
    $query = "SELECT * FROM umkm";
} else {
    // Pastikan kategori aman dari SQL Injection
    $kategori = $conn->real_escape_string($kategori);
    $query = "SELECT * FROM umkm WHERE kategori = '$kategori'";
}

$result = $conn->query($query);

// Mendapatkan daftar kategori untuk dropdown
$kategori_query = "SELECT DISTINCT kategori FROM umkm";
...
<body>
    ...
    <main>
        <div class="container mt-4">
            <h2>UMKM</h2>
            <form method="GET" action="UMKM.php">
            ...
            <div id="daftar-umkm" class="mt-4">
                <?php while ($row = $result->fetch_assoc()) { ?>
                    <div class="umkm-item">
                        <img src="admin/new_images/umkm/<?php echo $row['gambar']; ?>" alt="Gambar UMKM">
                        <div>
                            <h3><?php echo $row['nama']; ?></h3>
                            <p><?php echo $row['deskripsi']; ?></p>
                            <a href="umkm_detail.php?id=<?php echo $row['id']; ?>" class="btn btn-primary">Kunjungi</a>
                        </div>
                    </div>
                <?php } ?>
            </div>
